 ...
 *     (Expired registrations stay; they get the "Inactive" pill.)
 *
 ...
 *   - `drep_voters` per-DRep — same reason.
...
/**
 * Build a `DRepDirectoryItem` from the three Koios sources. The `info`
 * row carries the lifecycle/voting fields; the `metadata` row carries
 * the CIP-119 body. Either may be missing — we still emit a row for
 * every registered DRep (the directory must include DReps without
 * anchors, which is the common case today).
 */
...
export async function runDirectorySync(): Promise<DirectorySyncResult> {
  ...
  // wouldn't render meaningfully in the directory.
  const registered = listing.filter((d) => d.registered);
  result.total = registered.length;
  console.log(
    `Directory sync: drep_list returned ${listing.length} (${registered.length} registered)`,
  );
  ...
  // Step 4: write rows. We do not look up the existing row to compare
  ...
  for (const id of drepIds) {
    try {
      ...
      const item = buildDirectoryItem(id, info, meta, now);
  ...
  console.log(
    `Directory sync complete: total=${result.total} written=${result.written} ` +
      `withMetadata=${result.withMetadata} withGivenName=${result.withGivenName} ` +
      `withImage=${result.withImage} errors=${result.errors}`,
  );
  return result;
}

/**
 * Tests for `clubhouse/flagComment.ts` — the Sprint 4 follow-up
 * community-flagging primitive for clubhouse COMMENTS.
 *
 * Closes the last leg of the Sprint 4 flagging trio. Sibling of
 * `flagPost.test.ts` and `comments/flag.test.ts` — same four claims
 * (a/b/c/d), different resource:
 *
 *   - per-flagger row lands in `clubhouse_comment_flags` keyed by
 *     (`postKey`, `commentFlagKey`),
 *   - denormalised `flagCount` / `hidden` mutate the
 *     `clubhouse_comments` row,
 *   - same `HIDE_THRESHOLD = 3` reused from `comments/flag.ts`.
 */
...
import { handler } from './flagComment';
...
const COMMENT_ID = 'comment-ulid-01';
const AUTHOR_WALLET = 'stake1ucommentauthorxxxxxxxxxxxxxxxxxxxxxxxxx';
...
function buildEvent(opts: {
  ...
  return {
    body: null,
    pathParameters: { drepId: DREP_ID, postId: POST_ID, commentId: COMMENT_ID },
    requestContext: {
      authorizer: {
        lambda: {
...
describe('clubhouse/flagComment', () => {
  beforeEach(() => {
    mockGet.mockReset();
  ...
  it('claim (a) — 3 distinct flaggers hide the comment (newCount=3 → hidden=true)', async () => {
    ...
    // Per-flagger row landed in `clubhouse_comment_flags` keyed by
    // `${commentId}#${flaggerId}` SK.
    expect(mockPutIfAbsent).toHaveBeenCalledTimes(1);
    const insertCall = mockPutIfAbsent.mock.calls[0]!;
    expect(insertCall[0]).toBe('test-clubhouse_comment_flags');
    const insertedRow = insertCall[1] as Record<string, unknown>;
    expect(insertedRow['postKey']).toBe(`${DREP_ID}#${POST_ID}`);
    expect(insertedRow['commentFlagKey']).toBe(
      `${COMMENT_ID}#stake1uflaggerthreexxxxxxxxxxxxxxxxxxxxxxxxxxx`,
    );
  ...
  it('claim (b) — duplicate flag from same wallet is idempotent (200 + already_flagged + no counter mutation)', async () => {
    ...
    expect(auditRow['eventType']).toBe('clubhouse.comment.flag_dup');
  });
  ...
  it('claim (d) — comment author cannot self-flag (400 before any write)', async () => {
    const SELF = 'stake1uselfcommentxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx';
    mockGet.mockResolvedValueOnce({
      postKey: `${DREP_ID}#${POST_ID}`,
      commentId: COMMENT_ID,
      authorWallet: SELF,
